...
export function SubscribePanel() {
	...
	const { data: currentPlans, isLoading: isLoadingPlans } =
		api.paykit.currentPlans.useQuery();
	const activePlan =
		currentPlans?.find((plan) =>
			["active", "trialing", "past_due"].includes(plan.status),
		) ?? null;
	const scheduledPlan =
		currentPlans?.find((plan) => plan.status === "scheduled") ?? null;
